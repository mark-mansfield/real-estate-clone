...
const FeaturedProperties = ({ title, data, slideSpeed }) => {
  const settings = {
    accessibility: true,
    dots: false,
    arrows: true,
  ...
  return (
    <div className=" mt-5 mb-5 featured-properties">
      <h2>{title}</h2>
      <Slider {...settings}>
        {data.map((property, idx) => {
          return (
            <div className="slide card" key={idx} style={{ height: '276px' }}>
              <a style={{ color: 'black' }} href={property.link} data-id={property.headline}>
                <span className="green-label">New</span>
                <div className="image-wrapper">
                  <img className="slide-img" alt={property.headline} src={property.image} style={{ height: '189px' }} />
                </div>
                <div className="header">
                  <h2 className="custom-h2">
                    <span className="region">{property.region}</span>
                    <span className="property-state">{property.state}</span>
                  </h2>
                  ...
                  <div className="property__agent">{property.agent}</div>
                </div>
              </a>
            </div>
          );
